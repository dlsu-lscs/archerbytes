...
export async function handleCategoryEvent(
  payload: CategoryWebhookPayload,
): Promise<void> {
  const { action, categoryId } = payload as CategoryWebhookPayload & {
    categoryId: string;
  };
  ...
  try {
    ...
    console.error(`[Webhook Handler] Failed to handle category:${action}:`, {
      categoryId,
      error,
    });
    throw error;
